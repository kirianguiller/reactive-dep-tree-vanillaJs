...
import { SentenceJson, MetaJson, TreeJson, TokenJson, FeatureJson } from 'conllup/lib/conll';
...
export const defaultSentenceSVGOptions = (): SentenceSVGOptions => ({ shownFeatures: ["FORM"], interactive: false });
...
export class SentenceSVG extends EventDispatcher {
  ...
  constructor(svgID: string, reactiveSentence: ReactiveSentence, sentenceSVGOptions: SentenceSVGOptions) {
    super();
  ...
  populateLevels(): void {
    ...
    this.levelsArray = Array.apply(null, Array(headsIdArray.length)).map(function () {
      return -1;
    });
    var level = 0;
  ...
  adaptSvgCanvas(): void {
    ...
    this.snapSentence.attr({ width: 500 });
  ...
  exportConll() {
    return sentenceJsonToConll({ treeJson: this.treeJson, metaJson: this.metaJson });
  }
...
class TokenSVG {
  ...
  createSnap(snapSentence: Snap.Paper, shownFeatures: string[], startX: number, startY: number): void {
    ...
    for (const feature of shownFeatures) {
      ...
      const snapFeature = snapSentence.text(this.startX, runningY, this.tokenJson[feature] as string);
  ...
  drawRelation(snapSentence: Snap.Paper, headCoordX: number, levelHeight: number): void {
    ...
    if (headCoordX == 0) {
      ...
      yTop = heightArc;
      xTo =
        this.tokenJson.ID > this.tokenJson.HEAD ? headCoordX + SVG_CONFIG.gapX / 2 : headCoordX - SVG_CONFIG.gapX / 2;
    ...
    var snapDeprel = snapSentence.text(deprelX, deprelY, this.tokenJson.DEPREL).addClass('DEPREL');
